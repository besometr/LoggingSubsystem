...
//
// Параметры:
//	СтруктураПользовательскихПолей	- Структура - Пользовательских данных лога, заполненная по значениями соответствующих полей.
...
Функция КонструкторСтруктурыЛога(СтруктураПользовательскихПолей)Экспорт 
	...
	СтруктураЛогаGELF			= ЛогированиеПовтИсп.СтруктураЛогаGELF();
	СтруктураУровнейЛога		= ЛогированиеПовтИсп.СтруктураУровнейЛога();
	СтруктураСлужебныхПолейЛога = ЛогированиеПовтИсп.СтруктураСлужебныхПолейЛога();
	СтруктураСеансовыхПолейЛога	= ЛогированиеПовтИсп.СтруктураСеансовыхПолейЛога();
	...
	СтруктураЛога.Вставить("_app",				СтруктураСлужебныхПолейЛога.app);
	СтруктураЛога.Вставить("_node",				СтруктураСлужебныхПолейЛога.node);
	СтруктураЛога.Вставить("_branch",			СтруктураСлужебныхПолейЛога.branch);
...
//
// Возвращаемое значение:
//	Структура			- Структура пользовательских данных лога, заполненная по значениями соответствующих полей.
//		* КороткоеСообщение			- Строка	- Короткое представление сообщения.
//		* ПолноеСообщение			- Строка	- Полное сообщение.
//		* ДатаСобытия				- Строка	- Дата и время события UTC 0.
//		* Уровень					- Число		- Уровень лога (ALL=1,FATAL=2,ERROR=3,WARN =4 ,INFO=6,DEBUG=7,TRACE=5).
...
//
Функция КонструкторСтруктурыПараметровПодключенияКELK(ТипНастройкиПодключенияКELK)Экспорт
			
	СтруктураПараметровПодключения = Новый Структура;
	
	СтруктураПараметровПодключения.Вставить("Сервер",	"");
	СтруктураПараметровПодключения.Вставить("Порт",		0);
	СтруктураПараметровПодключения.Вставить("Индекс",	"");
	СтруктураПараметровПодключения.Вставить("Тип",		"");
	СтруктураПараметровПодключения.Вставить("Таймаут",	0);
	
	Если ТипЗнч(ТипНастройкиПодключенияКELK) = Тип("ПеречислениеСсылка.ТипыНастроекПодключенияКELK") Тогда 
		ЛогированиеВызовСервера.ЗаполнитьПараметрыПодключенияКELK(СтруктураПараметровПодключения, ТипНастройкиПодключенияКELK);
	КонецЕсли;	
...
// ИнициализироватьСтруктуруПараметровПодключенияКELK_ДляЗаписи
// Формирует структуру параметров подключения для дальнейшей записи настроек.
//
// Параметры:
...
//
// Возвращаемое значение:
//	Структура	- Параметры подключения к ELK.
//		* ТипНастройки	- ПеречислениеСсылка.ТипыНастроекПодключенияКELK - Тип настройки (Для чтения или для записи).
//		* Сервер		- Строка	- Сервер ELK.
//		* Порт			- Число		- Порт ELK.
//		* Индекс		- Строка	- Индекс в ElasticSearch (аналог БД).
...
//		* Таймаут		- Число		- Ожидание выполнения запроса.
//
Функция ИнициализироватьСтруктуруПараметровПодключенияКELK_ДляЗаписи(ТипНастройки, Сервер, Порт, Индекс, Тип, Таймаут = 0)Экспорт 
	
	СтруктураПараметровЗаписи = Новый Структура();
	
	СтруктураПараметровЗаписи.Вставить("ТипНастройки",	ТипНастройки);
	СтруктураПараметровЗаписи.Вставить("Сервер",		Сервер);
	СтруктураПараметровЗаписи.Вставить("Порт",			Порт);
	СтруктураПараметровЗаписи.Вставить("Индекс",		Индекс);
	СтруктураПараметровЗаписи.Вставить("Тип",			Тип);
	СтруктураПараметровЗаписи.Вставить("Таймаут",		Таймаут);
	
	Возврат СтруктураПараметровЗаписи;
...
//								Значение - Критерий поиска.
//	СтруктураСортировки			- Структура	- Ключ - Имя поля, по которому будет осуществляться сортировка, 
...
Функция КонструкторСтруктурыПоискаЛогов(МассивВыбранныхПолей,
	...
	Для Каждого КлючИЗначение Из СтруктураБольшеИлиРавно Цикл 
		СоответствиеТипаОтбора			= Новый Соответствие;
		СоответствиеОтбораПоПолю		= Новый Соответствие;
		СоответствиеОтбораБольшеИлиРавно= Новый Соответствие;
		
		СоответствиеОтбораБольшеИлиРавно.Вставить("gte", КлючИЗначение.Значение);
		
		СоответствиеОтбораПоПолю.Вставить(КлючИЗначение.Ключ, СоответствиеОтбораБольшеИлиРавно);
	...
	Для Каждого КлючИЗначение Из СтруктураМеньшеИлиРавно Цикл 
		СоответствиеТипаОтбора			= Новый Соответствие;
		СоответствиеОтбораПоПолю		= Новый Соответствие;
		СоответствиеОтбораМеньшеИлиРавно= Новый Соответствие;
		
		СоответствиеОтбораМеньшеИлиРавно.Вставить("lte", КлючИЗначение.Значение);
		
		СоответствиеОтбораПоПолю.Вставить(КлючИЗначение.Ключ, СоответствиеОтбораМеньшеИлиРавно);
	...
	ИначеЕсли ТипЗнч(КоличествоЗаписей) = Тип("Строка")
		И ОбщегоНазначенияВызовСервера.ЭтоЧисло(КоличествоЗаписей) Тогда 
