...
Процедура ИнициализацияПолейОтбора()
	
	МассивДоступныхВидовСравнения_Строка = Новый Массив;
	МассивДоступныхВидовСравнения_Строка.Добавить(ВидСравненияКомпоновкиДанных.Равно);
	МассивДоступныхВидовСравнения_Строка.Добавить(ВидСравненияКомпоновкиДанных.Содержит);
	
	Для Каждого ДоступноеПолеОтбора Из КомпоновщикНастроек.Настройки.ДоступныеПоляОтбора.Элементы Цикл 
		
		Если ДоступноеПолеОтбора.ТипЗначения = Тип("Строка") Тогда 
			ДоступноеПолеОтбора.ДоступныеВидыСравнения.ЗагрузитьЗначения(МассивДоступныхВидовСравнения_Строка);
		КонецЕсли;
	КонецЦикла;
